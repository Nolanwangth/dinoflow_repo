...
def evaluate_validation_split(
    ...
    n_batches = max(1, math.ceil(cfg.val_num_frames / val_bs))
    action_dim = int(unwrapped.config.action_dim)
    ...
    for _ in range(n_batches):
        batch = next(dl_iter)
        for cam_key in val_dataloader.dataset.meta.camera_keys:
...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    # Validation split: held-out LeRobotDataset, evaluated on the main process
    # only. Reuses the training preprocessor/postprocessor (train stats) so the
    # val metrics are directly comparable to training.
    val_dataloader = None
    if cfg.validation_dataset is not None and cfg.val_freq > 0 and is_main_process:
    ...
    for _ in range(step, cfg.steps):
        ...
        is_val_step = cfg.validation_dataset is not None and cfg.val_freq > 0 and step % cfg.val_freq == 0
        ...
        if is_val_step:
            if is_main_process:
                ...
                val_metrics = evaluate_validation_split(
                    cfg=cfg,
                    accelerator=accelerator,
                    policy=policy,
                    preprocessor=preprocessor,
