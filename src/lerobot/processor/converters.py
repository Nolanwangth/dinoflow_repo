...
def batch_to_transition(batch: dict[str, Any]) -> EnvTransition:
    ...
    # Also capture "next.observation.*" keys so they flow through the pipeline.
    next_obs_keys = {k: v for k, v in batch.items() if k.startswith("next.observation.")}
    observation_keys.update(next_obs_keys)
    # Also capture "future.observation.*" keys for multi-step touch dreaming.
    future_obs_keys = {k: v for k, v in batch.items() if k.startswith("future.observation.")}
    observation_keys.update(future_obs_keys)
