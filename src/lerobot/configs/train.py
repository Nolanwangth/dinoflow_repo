...
class TrainPipelineConfig(HubMixin):
    ...
    # Optional held-out LeRobotDataset split for periodic validation during
    # training (no gym env required). Uses the same policy/preprocessor as
    # training (train normalization stats), so metrics are directly comparable.
    validation_dataset: DatasetConfig | None = None
    # How often (in training steps) to run validation. 0 disables it.
    val_freq: int = 5_000
