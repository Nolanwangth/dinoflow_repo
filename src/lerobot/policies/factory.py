...
from .act_htd.configuration_act_htd import ACTHTDConfig
from .bbvla.configuration_bbvla import BbvlaConfig
from .diffusion.configuration_diffusion import DiffusionConfig
from .dino_flow.configuration_dino_flow import DinoFlowConfig
...
def get_policy_class(name: str) -> type[PreTrainedPolicy]:
    ...
    if name == "tdmpc":
        ...
        from .dino_flow.modeling_dino_flow import DinoFlowPolicy

        return DinoFlowPolicy
    elif name == "act":
        from .act.modeling_act import ACTPolicy

        return ACTPolicy
    elif name == "act_htd":
        from .act_htd.modeling_act_htd import ACTHTDPolicy
        ...
        from .bbvla.modeling_bbvla import BbvlaPolicy
...
def make_policy_config(policy_type: str, **kwargs) -> PreTrainedConfig:
    ...
    elif policy_type == "dino_flow":
        return DinoFlowConfig(**kwargs)
    elif policy_type == "act":
        return ACTConfig(**kwargs)
    elif policy_type == "act_htd":
        return ACTHTDConfig(**kwargs)
    ...
    elif policy_type == "bbvla":
        return BbvlaConfig(**kwargs)
...
def make_pre_post_processors(
    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    """
    # Skip loading pretrained processors if the checkpoint is from a different policy type
    # (e.g., loading bbvla with a pi05_base checkpoint — processor architectures differ).
    _skip_pretrained_processor = False
    # Import custom processor modules before attempting to load a serialized
    # pipeline.  The module-level decorator registers custom steps such as
    # dino_flow_slice_features in ProcessorStepRegistry.
    if isinstance(policy_cfg, DinoFlowConfig):
        from .dino_flow import processor_dino_flow  # noqa: F401

    if pretrained_path and isinstance(policy_cfg, BbvlaConfig):
        try:
            _ckpt_cfg = PreTrainedConfig.from_pretrained(
                pretrained_path, local_files_only=True
            )
            _skip_pretrained_processor = (_ckpt_cfg.type != "bbvla")
        except Exception:
            # If we can't read the checkpoint config (e.g. pi05_base is a local
            # directory alias, not a full HF repo), skip loading its processor
            # and use bbvla's own processor below.
            _skip_pretrained_processor = True
    if pretrained_path and not _skip_pretrained_processor:
    ...
    elif isinstance(policy_cfg, DinoFlowConfig):
        from .dino_flow.processor_dino_flow import make_dino_flow_pre_post_processors

        processors = make_dino_flow_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )
    ...
    elif isinstance(policy_cfg, ACTHTDConfig):
        from .act_htd.processor_act_htd import make_act_htd_pre_post_processors

        processors = make_act_htd_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )
    ...
    elif isinstance(policy_cfg, BbvlaConfig):
        from .bbvla.processor_bbvla import make_bbvla_pre_post_processors

        processors = make_bbvla_pre_post_processors(
            config=policy_cfg,
            dataset_stats=kwargs.get("dataset_stats"),
        )
