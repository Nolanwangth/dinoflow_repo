...
class DatasetReader:
    ...
    def _query_hf_dataset(self, query_indices: dict[str, list[int]]) -> dict:
        ...
        "future.observation.*" keys are remapped to "observation.*" so the
        dataset can dynamically fetch future frames without pre-computed columns.
        """
        result: dict = {}
        for key, q_idx in query_indices.items():
            ...
            # Remap "future.observation.*" → "observation.*" for dynamic future-state fetch
            hf_key = key.replace("future.observation.", "observation.") if key.startswith("future.") else key
            try:
                result[key] = torch.stack(self.hf_dataset[hf_key][relative_indices])
