...
from lerobot.utils.constants import ACTION, FUTURE_OBS_STATE, IMAGENET_STATS, OBS_PREFIX, OBS_STATE, REWARD
...
def resolve_delta_timestamps(
    ...
    for key in ds_meta.features:
        ...
        if key == OBS_STATE:
            history_indices = getattr(cfg, "contact_history_delta_indices", None)
            indices = history_indices if history_indices is not None else cfg.observation_delta_indices
    ...
    # Handle future_observation_delta_indices (e.g. BBVLA touch dreaming multi-step)
    future_delta = getattr(cfg, "future_observation_delta_indices", None)
    if future_delta is not None:
        delta_timestamps[FUTURE_OBS_STATE] = [i / ds_meta.fps for i in future_delta]
...
def make_dataset(
    cfg: TrainPipelineConfig, dataset_cfg: DatasetConfig | None = None
) -> LeRobotDataset | MultiLeRobotDataset:
    """Handles the logic of setting up delta timestamps and image transforms before creating a dataset.

    Args:
        cfg (TrainPipelineConfig): A TrainPipelineConfig config which contains a DatasetConfig and a PreTrainedConfig.
        dataset_cfg (DatasetConfig | None): Optional override for the DatasetConfig to load.
            Used for validation splits: the same delta-timestamp / image-transform logic is applied
            against a separate held-out dataset, while delta timestamps still come from
            ``cfg.trainable_config`` (the policy).
    ...
    Returns:
        LeRobotDataset | MultiLeRobotDataset
    """
    dataset_cfg = dataset_cfg or cfg.dataset
    image_transforms = (
